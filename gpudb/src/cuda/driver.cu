...
/* For initializing CUDA device */
	...
	for(int i=0;i<blockTotal;i++){
		...
		partTable->totalAttr = 2;
		partTable->attrType = (int *) malloc(sizeof(int)*2);
		CHECK_POINTER(partTable->attrType);
		partTable->attrSize = (int *) malloc(sizeof(int)*2);
		CHECK_POINTER(partTable->attrSize);
		partTable->attrIndex = (int *) malloc(sizeof(int)*2);
		CHECK_POINTER(partTable->attrIndex);
		partTable->attrTotalSize = (int *) malloc(sizeof(int)*2);
		CHECK_POINTER(partTable->attrTotalSize);
		partTable->dataPos = (int *) malloc(sizeof(int)*2);
		CHECK_POINTER(partTable->dataPos);
		partTable->dataFormat = (int *) malloc(sizeof(int)*2);
		CHECK_POINTER(partTable->dataFormat);
		partTable->content = (char **) malloc(sizeof(char *)*2);
		CHECK_POINTER(partTable->content);
		partTable->attrSize[0] = 9;
		partTable->attrIndex[0] = 4;
		partTable->attrType[0] = STRING;
		partTable->dataPos[0] = MMAP;
		outFd = open("PART4",O_RDONLY);
		offset = i * sizeof(struct columnHeader) + tupleOffset *9;
		lseek(outFd,offset,SEEK_SET);
		read(outFd,&header, sizeof(struct columnHeader));
		offset += sizeof(struct columnHeader);
		partTable->dataFormat[0] = header.format;
		outSize = header.tupleNum * 9;
		...
		diskTotal += (diskEnd.tv_sec -  diskStart.tv_sec)* BILLION + diskEnd.tv_nsec - diskStart.tv_nsec;
		close(outFd);
		partTable->tupleSize = 0 + 9 + 9 + sizeof(int);
		...
		(partRel.filter)->exp[0].relation = GEQ;
		strcpy((partRel.filter)->exp[0].content,"MFGR#2221");

		(partRel.filter)->exp[1].index = 0;
		(partRel.filter)->exp[1].relation = LEQ;
		strcpy((partRel.filter)->exp[1].content,"MFGR#2228");
	...
	for(int i=0;i<blockTotal;i++){
		...
		diskTotal += (diskEnd.tv_sec -  diskStart.tv_sec)* BILLION + diskEnd.tv_nsec - diskStart.tv_nsec;
		close(outFd);
		supplierTable->attrSize[1] = 12;
		supplierTable->attrIndex[1] = 5;
		supplierTable->attrType[1] = STRING;
		...
		diskTotal += (diskEnd.tv_sec -  diskStart.tv_sec)* BILLION + diskEnd.tv_nsec - diskStart.tv_nsec;
		close(outFd);
		supplierTable->tupleSize = 0 + sizeof(int) + sizeof(int) + 12;
		...
		supplierRel.outputNum = 1;
		supplierRel.outputIndex = (int *)malloc(sizeof(int) * 1);
		CHECK_POINTER(supplierRel.outputIndex);
		supplierRel.outputIndex[0] = 0;
		supplierRel.whereIndex[0] = 1;
	...
	for(int i=0;i<blockTotal;i++){
		...
		if(blockTotal != 1){
			mergeIntoTable(ddateRes,ddateTable,&pp);
			clock_gettime(CLOCK_REALTIME,&diskStart);
			freeTable(ddateTable);
			clock_gettime(CLOCK_REALTIME,&diskEnd);
			diskTotal += (diskEnd.tv_sec -  diskStart.tv_sec)* BILLION + diskEnd.tv_nsec - diskStart.tv_nsec;
		}else{
			free(ddateRes);
	...
	for(int i=0;i<blockTotal;i++){
		...
		jNode0.rightPos[0] = 0;
		jNode0.tupleSize += partRes->attrSize[0];
		...
		jNode1.totalAttr = 3;
		jNode1.keepInGpu = (int *) malloc(sizeof(int) * 3);
		CHECK_POINTER(jNode1.keepInGpu);
		for(int k=0;k<3;k++)
			jNode1.keepInGpu[k] = 1;
		jNode1.rightOutputAttrNum = 0;
		...
		jNode1.rightOutputAttrType = (int *)malloc(sizeof(int)*0);
		CHECK_POINTER(jNode1.rightOutputAttrType);
		jNode1.rightOutputIndex = (int *)malloc(sizeof(int)*0);
		CHECK_POINTER(jNode1.rightOutputIndex);
		jNode1.rightPos = (int *)malloc(sizeof(int)*0);
		...
		jNode2.totalAttr = 3;
		jNode2.keepInGpu = (int *) malloc(sizeof(int) * 3);
		CHECK_POINTER(jNode2.keepInGpu);
		for(int k=0;k<3;k++)
			jNode2.keepInGpu[k] = 1;
		jNode2.rightOutputAttrNum = 1;
		jNode2.leftOutputAttrNum = 2;
		jNode2.leftOutputAttrType = (int *)malloc(sizeof(int)*2);
		CHECK_POINTER(jNode2.leftOutputAttrType);
		jNode2.leftOutputIndex = (int *)malloc(sizeof(int)*2);
		CHECK_POINTER(jNode2.leftOutputIndex);
		jNode2.leftPos = (int *)malloc(sizeof(int)*2);
	...
	gbNode->gbExp[0].exp.opValue = 2;
	gbNode->attrType[1] = result->attrType[0];
	gbNode->attrSize[1] = result->attrSize[0];
	...
	gbNode->attrType[2] = result->attrType[1];
	gbNode->attrSize[2] = result->attrSize[1];
